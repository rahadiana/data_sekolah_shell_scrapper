...
cat  cache/tmp/ kodewilayah.txt | sed "s/^/sudo mkdir -p data_sekolah\/province\/$bbb\//" | bash
sudo mkdir -p data_sekolah/city/$bbb
cat  cache/tmp/ kodewilayah.txt|sed "s/ //g" | sed "s/^/wget -nc -q 'https:\/\/dapo.kemdikbud.go.id\/rekap\/dataSekolah?id_level_wilayah=1\&kode_wilayah=/; s/$/' -O data_sekolah\/province\/$bbb\//" > cache/tmp/ city.txt
paste -d "" cache/tmp/ city.txt  cache/tmp/ kodewilayah.txt cache/tmp/ kodewilayah.txt | sed 's/$/.json/' | sed 's/  .json/.json/g; s/  /\//g' | bash
sudo mkdir -p cache/tmp/ data_sekolah
...
FindCity(){
echo "findCity start"
cat cache/tmp/ fin.json | jq -r '.[].kode_wilayah' | sed "s/ //g; s/^/sudo mkdir -p data_sekolah\/city\/$bbb\//" | bash
ls data_sekolah/city/$bbb |sed "s/ //g" > cache/tmp/ data_sekolah/city.txt
cat cache/tmp/ data_sekolah/city.txt | sed "s/^/timeout 10 wget -nc -q 'https:\/\/dapo.kemdikbud.go.id\/rekap\/dataSekolah?id_level_wilayah=2\&kode_wilayah=/; s/$/\&smester_id=$bbb' -O data_sekolah\/city\/$bbb/"  > cache/tmp/ data_sekolah/city2.txt
...
cat cache/tmp/ allkota.txt | sed "s/^/sudo mkdir -p data_sekolah\/region\/$bbb\//g" | bash
cat cache/tmp/ allkota.txt | sed "s/^/timeout 10 wget -nc -q 'https:\/\/dapo.kemdikbud.go.id\/rekap\/progresSP?id_level_wilayah=3\&kode_wilayah=/; s/$/\&smester_id=$bbb\&bentuk_pendidikan_id=' -O data_sekolah\/region\/$bbb/" > cache/tmp/ data_sekolah/region.txt
...
#sudo mkdir -p  cache/tmp/ data_sekolah/region/$bbb
#sudo mkdir -p cache/tmp/ data_sekolah/region/cache
#ls data_sekolah/region/$bbb > cache/tmp/ data_sekolah/region/reg_dmp.txt
#cat cache/tmp/ data_sekolah/region/reg_dmp.txt | sed -e "s/^/cp data_sekolah\/region\/$bbb\//" > cache/tmp/ data_sekolah/region/reg_dmp2.txt
#paste -d "/" cache/tmp/ data_sekolah/region/reg_dmp2.txt cache/tmp/ data_sekolah/region/reg_dmp.txt | sed 's/$/.json \/tmp\/data_sekolah\/region\/cache\//' > cache/tmp/ data_sekolah/region/reg_dmp3.txt 
#paste -d "" cache/tmp/ data_sekolah/region/reg_dmp3.txt cache/tmp/ data_sekolah/region/reg_dmp.txt |sed 's/$/.json/' | bash



#05
#27
#cat  cache/tmp/ data_sekolah/region/cache/0511*.json | jq '.'

#ls cache/tmp/ data_sekolah/region/cache/05*.json | wc -l